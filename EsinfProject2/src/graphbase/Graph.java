...
public class Graph<V, E> implements GraphInterface<V, E> {
    ...
    public Iterable<Edge<V, E>> edges() {
        LinkedList<Edge<V, E>> lista = new LinkedList<>();
        for (V v : vertices.keySet()) {
            for (V v1 : vertices.keySet()) {
                if (!v.equals(v1)) {
                    if (getEdge(v, v1) != null) {
                        lista.add(getEdge(v, v1));
                    }
                }
            }
        }
    ...
    public Iterable<Edge<V, E>> incomingEdges(V vert) {
        ...
        LinkedList<Edge<V, E>> lista = new LinkedList<>();
        for (V vAdj : vertices.keySet()) {
            if (getEdge(vAdj, vert) != null) {
                lista.add(getEdge(vAdj, vert));
            }
        }
        return lista;
    }
