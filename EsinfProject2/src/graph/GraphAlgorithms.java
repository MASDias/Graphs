...
public class GraphAlgorithms {
    ...
    public static <V, E> LinkedList<V> BFS(AdjacencyMatrixGraph<V, E> graph, V vertex) {
        ...
        LinkedList<V> queueVertices = new LinkedList();
        ...
        while (!queueAux.isEmpty()) {
            V deleted = queueAux.removeFirst();
            for (V vAdj : graph.directConnections(deleted)) {
                if (!visited[graph.toIndex(vAdj)]) {
                    queueVertices.add(vAdj);
                    queueAux.add(vAdj);
                    visited[graph.toIndex(vAdj)] = true;
                }
            }
        }
        return queueVertices;
    }
